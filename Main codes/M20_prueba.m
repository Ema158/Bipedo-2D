...
% Options
% ----------------------------------------------- 
DataName = 'InfoBiped_Param_t_10prueba3'; % File produced in "M04d_...m" CHANGE OptionContVar = 1;
...
% -----------------------------------------------
robot = InfBipedo.robot;
% robot = genebot(); 
% initParam = InfROMEO.iniParameters;
gait_parameters = InfBipedo.gait_parameters;
Solution = InfBipedo.Solution;
...
%     Q = state_v(robot);
    ...
    if mod(j,2) % If j is impair 
        ...
        qtSwap = [-qt{j}(6,:); -qt{j}(5,:); -qt{j}(4,:);-qt{j}(3,:);-qt{j}(2,:);-qt{j}(1,:)];
        qDtSwap = [-qDt{j}(6,:); -qDt{j}(5,:); -qDt{j}(4,:);-qDt{j}(3,:);-qDt{j}(2,:);-qDt{j}(1,:)];
        qDDtSwap = [-qDDt{j}(6,:); -qDDt{j}(5,:); -qDDt{j}(4,:);-qDDt{j}(3,:);-qDDt{j}(2,:);-qDDt{j}(1,:)];
...
%% Evolution of the key point variables (29 controlled variables and X-Y CoM)
disp('--------------------------------------------------------------------------------------------');
